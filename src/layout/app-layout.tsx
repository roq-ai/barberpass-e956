...
import { getApps } from 'apiSdk/apps';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'barbershop-owner', 'barbershop-manager', 'barber', 'support'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AppInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/apps` : null),
    () => getApps({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'App',
      path: isTenantUser ? `/apps/view/${data?.[0]?.id}` : '/apps',
      entity: 'app',
    ...
    { name: 'Service', path: '/services', entity: 'service', service: AccessServiceEnum.PROJECT, icon: FiClipboard },
    {
      name: 'Subscription',
      path: '/subscriptions',
      entity: 'subscription',
