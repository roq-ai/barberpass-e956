...
import { getBarbershops } from 'apiSdk/barbershops';
...
function ServiceCreatePage() {
  ...
  const formik = useFormik<ServiceInterface>({
    initialValues: {
      name: '',
      description: '',
      barbershop_id: (router.query.barbershop_id as string) ?? null,
    },
    ...
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <form onSubmit={formik.handleSubmit}>
          ...
          <AsyncSelect<BarbershopInterface>
            formik={formik}
            name={'barbershop_id'}
            label={'Select Barbershop'}
            placeholder={'Select Barbershop'}
            fetcher={getBarbershops}
            renderOption={(record) => (
