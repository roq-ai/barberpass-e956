...
import { getBarbershopById } from 'apiSdk/barbershops';
...
function BarbershopViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BarbershopInterface>(
    () => (id ? `/barbershops/${id}` : null),
    () =>
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          <Text as="h1" fontSize="2xl" fontWeight="bold">
            Barbershop Detail View
          </Text>
          {hasAccess('barbershop', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/barbershops/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/appointments/create?barbershop_id=${data?.id}`}>
              ...
              {hasAccess('service', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/services/create?barbershop_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
